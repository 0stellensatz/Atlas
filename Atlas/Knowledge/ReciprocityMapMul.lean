...
The endpoint of the multiplicativity calculation: the Frobenius
conjugation, finite-stage unit, correction-sum, and universal
norm-descent results assemble into multiplicativity of the abstract
reciprocity map on Frobenius elements (#104).

## Main statements

* `DegreeData.reciprocityMap_mul` — reciprocity multiplicativity;
  proved.
...
/-- **Reciprocity multiplicativity**: the reciprocity function is
multiplicative on Frobenius elements, written additively on the
...
theorem reciprocityMap_mul
  ...
  letI hIfinite : Finite
    ...
    D.maximalUnramifiedExtension_finite KR.field L
