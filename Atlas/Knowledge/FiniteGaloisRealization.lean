...
`AlgebraicNumberTheory` prefix. The `OfEmbedding` half is generalized
over an ambient Galois extension `Ω` in place of the source's pinned
separable closure, so the algebraic-closure instantiation carrying the
local data can enter; the chosen half is unchanged at the separable
closure. Everything else ports token-for-token; the file is the
source's `LocalReciprocity/FiniteGaloisRealization.lean`.
...
noncomputable instance finiteGaloisFieldRangeOfEmbedding_finiteDimensional
  ...
  (finiteGaloisFieldRangeEquivOfEmbedding
...
noncomputable instance finiteGaloisExtensionSubgroupOfEmbedding_normal
    ...
    ((finiteGaloisClosedFixingSubgroupOfEmbedding
        K L Ω i).toSubgroup.subgroupOf
      (closedFixingSubgroup
        (⊥ : IntermediateField K Ω)).toSubgroup).Normal := by
  change
    ((closedFixingSubgroup
        (finiteGaloisFieldRangeOfEmbedding K L Ω i)).toSubgroup.subgroupOf
      (closedFixingSubgroup
        (⊥ : IntermediateField K Ω)).toSubgroup).Normal
  infer_instance
...
def finiteGaloisUnitsEquivAbstractFixedOfEmbedding
    ...
    Additive Lˣ ≃+
      ambientFixedAddSubgroup
        ...
        (finiteGaloisClosedFixingSubgroupOfEmbedding K L Ω i) :=
  embeddedFieldUnitsEquivGaloisFixed K Ω L i
...
theorem finiteGaloisUnitsEquivAbstractFixedOfEmbedding_coe
    (i : L →ₐ[K] Ω) (x : Lˣ) :
    (finiteGaloisUnitsEquivAbstractFixedOfEmbedding K L Ω i
      (Additive.ofMul x)).1 =
    ...
    ((closedFixingSubgroup
      ...
      (finiteGaloisClosedFixingSubgroupOfEmbedding
        ...
        (closedFixingSubgroup
          (⊥ : IntermediateField K Ω)).toSubgroup) ≃*
      (L ≃ₐ[K] L) :=
  (baseFixingExtensionQuotientEquivGaloisGroup K Ω
    (finiteGaloisFieldRangeOfEmbedding K L Ω i)).trans
      (AlgEquiv.autCongr
...
def finiteGaloisUnitsEquivAbstractFixed :
  ...
  finiteGaloisUnitsEquivAbstractFixedOfEmbedding K L (SeparableClosure K)
    (separableEmbeddingIntoSeparableClosure K L)
