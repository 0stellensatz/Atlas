...
mixed-characteristic local field, which is exactly its local compactness — is recorded
ahead of its proof. This item is the junction the reciprocity phase crosses every time it
evaluates a local notion at a place of a global field; without it,
`Atlas.Knowledge.IsLocalHilbertSymbol` cannot even be stated at a completion.
...
all — not even `ℚ_[p]` — and the missing content for the completion is Serre's
Proposition 1: locally compact iff complete with finite residue field. The source builds
the same certificate over its normed completion from its own machinery
...
half that is provable now ([Serre 1979, Chap. II, §1, p.27][Serre1979]). -/
...
field. The content is Serre's Proposition 1 — a complete discretely valued field is
locally compact iff its residue field is finite — plus the finiteness of that residue
field; the pinned Mathlib has no local-compactness for any nonarchimedean completion, so
the certificate is recorded ahead of its proof
