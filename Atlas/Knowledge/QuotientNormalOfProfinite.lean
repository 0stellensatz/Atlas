...
/-!
# normality of a residue extension under a profinite action

The profinite version of Mathlib's `Ideal.Quotient.normal`: for a profinite
group acting continuously on a discrete ring with invariant subring `A`, a
maximal residue extension `B/Q` over `A/P` is normal. Every element has a
finite orbit — its stabilizer is open, so an open normal subgroup fixes it —
and the orbit polynomial computed in the finite quotient splits its minimal
polynomial. This is the normality input of the residue-action exact sequence
in the reciprocity engine's instantiation (#104).

## Main statements

* `quotient_normal_of_profinite` — the residue extension is normal; proved.
