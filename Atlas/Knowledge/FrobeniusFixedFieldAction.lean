...
conjugation-stable action, it commutes with relative norms in a
power-fixed-field tower, and under inclusion into the maximal unramified
field it becomes the actual quotient action — power sums included (#104).

## Main definitions

* `DegreeData.frobeniusFixedFieldAction` — the induced action on the
  fixed field.

## Main statements

* `DegreeData.relativeNorm_frobeniusFixedFieldAction` — the relative
  norm is equivariant for every commuting quotient element; proved.
* `DegreeData.frobeniusFixedFieldAction_inclusion` — inclusion
  intertwines the action with the quotient action on the maximal
  unramified field; proved.
* `DegreeData.fixedFieldPowerSum_inclusion` — the fixed-field power sum
  becomes the global Frobenius power sum after inclusion; proved.
...
theorem frobeniusFixedFieldAction_coe_of_mk (D : DegreeData G) (A : Rep ℤ G)
  ...
  have hrField : (t⁻¹ * k).1 ∈
    ...
    let rI : (D.fieldInertia L).toSubgroup :=
      ⟨(t⁻¹ * k).1, ⟨Subgroup.mem_subgroupOf.1 hrel.1, hrel.2⟩⟩
