...
`finite_extension_trans`. The ambient group sits in one `{G : Type}`
scope: everything consumes the Type-pinned
`Atlas.Knowledge.DegreeData.finiteReciprocityHom`. All eight
...
totally ramified chain consumes it. The five group-theoretic privates
stay private with the source's scopes; the solvable induction keeps the
source's `termination_by`/`decreasing_by` on the quotient's
cardinality. The file is the source's `Reciprocity/Main.lean` through
its `ValuationData` section (`:28`–`:845`); the `abstractReciprocityEquiv`
and norm-residue-symbol tail and the naturality section are the next
bricks.
