...
after `Atlas.Knowledge.exists_classField`. The divisibility `(p - 1) ∣ e` is the source's
standing `e ∈ (p - 1) ℤ_{≥1}` for its §5's realizability discussion: a field containing a
`p`-th root of unity has ramification index divisible by `p - 1`, so without it nothing is
realizable and the claim would be false. The conclusion quantifies over every module structure
and unit filtration of the produced field, the honest reading of "`(I_K, β_K`) `= (I, β)`";
`Atlas.Knowledge.IsUnitFiltration` says why the quantification costs nothing.
...
index—a totally ramified extension of `ℚ_{p^f} (ζ_p)` in the source's construction. Claim
recorded ahead of its proof ([Pagano 2022, Thm. 5.4, pp.442–443][Pagano2022]). -/
theorem jumpSetRealization (p e f : ℕ+) [Fact (p : ℕ).Prime] (hp1 : 1 < p)
    ...
    ∃ (K : Type) (_ : Field K) (_ : ValuativeRel K) (_ : TopologicalSpace K)
      ...
      residueCharacteristic K = p ∧ absoluteInertiaDegree K = f ∧
        absoluteRamificationIndex K = e ∧
