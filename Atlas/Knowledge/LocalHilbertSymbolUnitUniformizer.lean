...
divisibility `n ∣ q - 1` is a hypothesis rather than a consequence of the roots of unity, so
that `Atlas.Knowledge.LocalHilbertSymbolTameFormula`, which proves it as
...
a unit of the integers with `K` holding the `n`-th roots of unity — every lift of `φ (ϖ)` is
the arithmetic Frobenius on the unramified field `K (u ^ (1 / n))`, so it raises a root of
`u` to the `q`-th power modulo the maximal ideal
([Serre 1979, Chap. XIV, §3, Prop. 8, p.210, proof p.211][Serre1979];
Yamaguchi 2026, `LocalClassFieldTheory/Kummer/PowerResidueTameFormula.lean:702`, in its
transposed slots and inverse normalization, and `:795`). -/
theorem localHilbertSymbol_unit_uniformizer {n : ℕ} {h : Kˣ → Kˣ → Kˣ}
  ...
  have hcval : valuation K ((h u ϖ : Kˣ) : K) = 1 := by
    have h1 : valuation K ((h u ϖ : Kˣ) : K) ^ n = 1 := by rw [← map_pow, hcpow, map_one]
    exact (pow_eq_one_iff.mp h1).resolve_right hn0
  ...
  have hmem := sub_pow_mem_maximalIdeal_of_isArithmeticFrobenius K F hfrob hn hdvd hu'
    ⟨γF, hγint⟩ hz hc
  set d : 𝒪[K] := ⟨_, hcint⟩ - ⟨(u : K), huint⟩ ^ ((Nat.card 𝓀[K] - 1) / n) with hddef
  have hne1 : valuation K (d : K) ≠ 1 := fun heq =>
  ...
  have hle1 : valuation K (d : K) ≤ 1 := (Valuation.mem_integer_iff _ _).mp d.2
  have hlt := lt_of_le_of_ne hle1 hne1
