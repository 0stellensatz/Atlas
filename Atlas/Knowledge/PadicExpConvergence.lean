...
`Atlas.Knowledge.span_residueCharacteristic_eq_maximalIdeal_pow`, and without it the threshold in
the statement would be about a length rather than about the ramification. The statements mention no
norm and no uniformity: the normed structure is rebuilt inside the proof from the local-field
hypotheses, as in `Atlas.Knowledge.IntegralClosureDVR`.
