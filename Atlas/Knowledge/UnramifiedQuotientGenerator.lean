...
subgroup, so every class is a power of the restriction. Surjectivity of the
normalized degree then supplies such a generator outright — the cyclicity
input of the unit-cohomology axiom (#104).
  ...
  have hzI : z ∈ D.fieldInertiaWithin K.field := by
    ...
    calc
      ...
      _ = ((Multiplicative.ofAdd (1 : ProfiniteInteger)) ^ n)⁻¹ *
        ...
        rw [hdt, hφ]
      _ = 1 := by simp
  have hzL : z.1 ∈ L.toSubgroup := hUnramified ⟨z.2, hzI⟩
