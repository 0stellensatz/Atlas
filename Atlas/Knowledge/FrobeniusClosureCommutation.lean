...
Commuting with the chosen Frobenius generator is commuting with its whole
closed procyclic closure — the centralizer of an element is closed, so it
absorbs the topological closure of the cyclic subgroup. Consequently a
quotient element commuting with the lift stabilizes the Frobenius fixed
field: conjugation by a representative carries `G_Σ` to itself (#104).

## Main statements

* `DegreeData.frobeniusClosure_commutes_of_commutes_generator` —
  commuting with the generator is commuting with `Γ`; proved.
...
theorem conjugate_frobeniusFixedField_eq_of_commutes (D : DegreeData G)
  ...
  · intro hx
    ...
    let yK : K.field.toSubgroup := ⟨k.1⁻¹ * x * k.1, by
      ...
      rw [← htx]
      exact K.field.toSubgroup.mul_mem
        (K.field.toSubgroup.mul_mem
          (K.field.toSubgroup.inv_mem k.2) htK) k.2⟩
    have hcomm := D.frobeniusClosure_commutes_of_commutes_generator
      K L hLK σ q hq ⟨QuotientGroup.mk t, htClosure⟩
    have hcomm' : q * QuotientGroup.mk t = QuotientGroup.mk t * q :=
      hcomm
