...
- **Different models for author and reviewer.** A reviewer sharing the author's model shares its blind spots; the review's value is exactly the failure modes the author does not have. What the rule fixes is the *difference*, and nothing else: any two capable models from different families satisfy it, from one vendor's line or across several, and which two a given run used belongs in that run's PR rather than in this document. Record the pairing where the work is—the PR body naming its author and its reviewer is what lets a later reader tell an unreviewed phase from a reviewed one.
